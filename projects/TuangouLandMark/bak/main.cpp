...
int main(int argc, char* argv[])
{
	...
	TermsProcessor* pProcessor = new TuanDanProcessor();;
	ret = pProcessor->WBProcessorInit(g_conf_file);
	if (ret < 0)
	{
		return -1;
	}
	nx_log(NGX_LOG_WARN,"Init end sucessfully!");
	ret = pProcessor->LoadTermsToIndex();
	if (ret < 0)
	{
		return -1;
	}
	nx_log(NGX_LOG_WARN,"load weibo sucessfully!");
	ret = pProcessor->FiltIndex(100);
	if (ret < 0)
	{
		return -1;
	}
	nx_log(NGX_LOG_WARN,"sort index sucessfully!");
	ret = pProcessor->DumpIndex();
	if (ret < 0)
	{
		return -1;
	}
	nx_log(NGX_LOG_WARN,"dump index sucessfully!");
	ret = pProcessor->DumpIndexAsText();
	if (ret < 0)
	{
		return -1;
	}
	nx_log(NGX_LOG_WARN,"dump index as text sucessfully!");
