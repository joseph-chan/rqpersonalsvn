...
int main(int argc, char* argv[])
{
	...
	ret = nx_log_init("./logs/makewordnetindex.",g_log_level);
	if (ret < 0)
	{
		fprintf(stderr,"init log error");
		return -1;
	...
	TermsProcessor* pWBProcessor = new TermsProcessor();;
	...
	ret = pWBProcessor->LoadTermsToIndex();
	if (ret < 0)
	{
		return -1;
	}
	nx_log(NGX_LOG_WARN,"load weibo sucessfully!");
	ret = pWBProcessor->FiltIndex(100);
	if (ret < 0)
	{
		return -1;
	}
	...
	ret = pWBProcessor->DumpIndexAsText();
	if (ret < 0)
	{
		return -1;
	}
