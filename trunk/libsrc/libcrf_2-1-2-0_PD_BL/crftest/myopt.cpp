...
int myopt_print_usage( const char* module, const char* author,const char* readme,
		const char *synopsis,const char *descr, myopt_t &map_opt )
{
	char linebuf[10240];
	snprintf( linebuf, sizeof(linebuf),	"\n\033[32m\033[1mNAME\033[0m\n\t%s - %s\n",module, readme);
	printf( linebuf );
	snprintf( linebuf, sizeof(linebuf), "\n\033[32m\033[1mSYNOPSIS\033[0m\n\t%s %s\n", module , synopsis );
	printf( linebuf );
	printf( "\n\033[32m\033[1mDESCRIPTION\033[0m\n" );

	int width = 80;
	for (int off = 0;off<(int)strlen(descr);off += width)
	{
		printf("\t%.*s\n",width,descr+off );
	}
	printf("\n");
	...
	while( it!=map_opt.end() )
	{
		...
		if ( '\0' != opt.ab ) 
			off = snprintf( linebuf, sizeof(linebuf),	"\t-%c --%s", opt.ab, opt.name);
		else 
			off = snprintf( linebuf, sizeof(linebuf),	"\t--%s", opt.name);

		switch (opt.opt_type)
		{
			case myoptInt:
				off += snprintf( linebuf+off, sizeof(linebuf)-off,   "=N");
				break;
			case myoptString:
				off += snprintf( linebuf+off, sizeof(linebuf)-off,   "=STRING");
				break;
			case myoptDate:
				off += snprintf( linebuf+off, sizeof(linebuf)-off,   "=YY-MM-DD[:YY-MM-DD]");
				break;
			case myoptDay:
				off += snprintf( linebuf+off, sizeof(linebuf)-off,   "=DAY");
				break;
			default:
				break;
		};
		...
		width = 80;
		int clen = strlen(opt.comment);
		if (opt.comment && clen>width)
		{
			int times = clen / width + (((clen%width) != 0) ?1:0) ;
			for (int i=0;i<times;i++)
			{

				if (0 == i)
					off += snprintf (linebuf+off,sizeof(linebuf)-off,"\n\t\t%.*s\n",width,opt.comment+i*width);
				else if (i != times-1)
					off += snprintf (linebuf+off,sizeof(linebuf)-off,"\t\t%.*s\n",width,opt.comment+i*width);
				else 
					off += snprintf (linebuf+off,sizeof(linebuf)-off,"\t\t%s %s\n\n"
							,opt.comment+i*width,srange);
			}


		} else {
			snprintf( linebuf+off, sizeof(linebuf)-off,	"\n\t\t%s %s\n\n", 
					opt.comment?opt.comment:"", srange);
		}
		printf( linebuf );
		it++;
	}
	printf( "\n\033[32m\033[1mAUTHOR\033[0m\n" );
	snprintf( linebuf, sizeof(linebuf),	"\tWritten by %s, %s.\n", 
