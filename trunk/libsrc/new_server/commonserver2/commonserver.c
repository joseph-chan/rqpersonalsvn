...
int g_threadNum=5; 
char g_tag_file[64];
dm_dict_t *g_dm_dict = NULL;
int g_tag_file_max_num= 655360;
...
{
	int ch;
	while ((ch = getopt(argc, argv, "vhdp:t:m:")) != -1)
	{
		switch (ch)
		{
			case 'v':		/* 打印版本信息 */
			...
			case 'p':
				...
				break;
			case 't':
				snprintf(g_tag_file,64,"%s",optarg);
				break;
			case 'm':
				g_tag_file_max_num = atoi(optarg);
				break;
			default:
				usage(argv[0], 1);
		}
