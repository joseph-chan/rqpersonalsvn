...
int main(int argc, char* argv[])
{
	...
	ret = nx_log_init("./logs/classifier.",g_log_level);
	if (ret < 0)
	{
		fprintf(stderr,"init log error");
		return -1;
	...
	NBClassifier* pNBC = new NBClassifier();;
	ret = pNBC->NBClassifierInit(g_conf_file);
	if (ret < 0)
	{
		return -1;
	}
	...
	nx_log(NGX_LOG_WARN,"Get NB model sucessfully!");
	ret = pNBC->NBPredict();
	if (ret < 0)
	{
		return -1;
	}
	nx_log(NGX_LOG_WARN,"predict sucessfully!");
