...
int WordNetIndexor::LoadIndexmmap(const char* word_index,const char* term_info,const char * relate_info,__gnu_cxx::hash_map<u_int,u_int> & TermIndex, vector<term_info_t> &TermInfo,file_mmap_t *file_mmap,total_index_info_t *total_info)
{
	FILE* fp_index1;
	FILE* fp_index2;
	FILE* fp_indexn;
	...
	if (NULL ==fp_index1 || NULL == fp_index2 || NULL == fp_indexn || NULL == fp_term_info )
	{
		nx_log(NGX_LOG_ERR,"load index error: can't not open file");
		return -1;
	}
	...
	fd = open(relate_info,O_RDONLY,0666);
	if(fd < 0)
	{
		nx_log(NGX_LOG_ERR,"read index error: can't not open file ! ret [%d]",fd);
		return -1;
	}
	filesize = lseek(fd,0,SEEK_END);
	if(filesize <= 0)
	{
		nx_log(NGX_LOG_ERR,"read index error: file size error ! ret [%d]",filesize);
		return -1;
	}
	...
	if (ptr == MAP_FAILED)
	{
		nx_log (NGX_LOG_ERR, "mmap %s fail", relate_info) ;
		return -1 ;
	}	

	file_mmap->fsize = filesize ;
	file_mmap->pmmap = ptr ;
	file_mmap->fd = fd ;
