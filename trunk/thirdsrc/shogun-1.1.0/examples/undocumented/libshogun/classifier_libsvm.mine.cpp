...
void get_rand_data()
{
	...
	feat=SG_MALLOC(char, NUM*DIMS);
	feat2=SG_MALLOC(char, NUM*DIMS);
...
int main()
{
	...
	SGSparseMatrix<int32_t> data=SGSparseMatrix<int32_t>(num_vectors,num_features);
	SGSparseMatrix<int32_t> data2=SGSparseMatrix<int32_t>(num_vectors,num_features);

	for (index_t i=0; i<num_vectors; ++i)
	{
		/* put elements only at even indices */
		data.sparse_matrix[i]=SGSparseVector<int32_t>(num_features, 2*i);
		data2.sparse_matrix[i]=SGSparseVector<int32_t>(num_features, 2*i);
		...
		for (index_t j=0; j<num_features; ++j)
		{
			...
			{	
				if (j< num_features /2)
				{
					data.sparse_matrix[i].features[j].entry=CMath::random(0,100) + DIST;
					data.sparse_matrix[i].features[j].feat_index= j;

					data2.sparse_matrix[i].features[j].entry=CMath::random(0,100) + DIST;
	...
	SG_REF(features);
	CSparseFeatures<int32_t>* features2 = new CSparseFeatures<int32_t>(data2);
	SG_REF(features2);
	...
	// test data
	//CSimpleFeatures<char>* features2 = new CSimpleFeatures<char>(feature_cache);
	//SG_REF(features2);
	//features2->set_feature_matrix(feat2, DIMS, NUM);
	...
	for (int32_t i=0; i<NUM; i++)
	{
		...
		for (index_t j=0; j<num_features; ++j)
		{
			printf("%d:%d,", data.sparse_matrix[i].features[j].feat_index,data.sparse_matrix[i].features[j].entry);
		}
