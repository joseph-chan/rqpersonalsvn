...
//this kind of code is not good, because it will generate too many code and big bin file.
//it is more reasonable to use template
...
void CAsciiFile::get_text_matrix(SGSparseVector<int64_t>*& matrix, int32_t& num_feat, int32_t& num_vec)	
{	
	size_t blocksize=1024*1024;	
	size_t required_blocksize=blocksize;	
	uint8_t* dummy=SG_MALLOC(uint8_t, blocksize);	
	...
	{	
		...
		blocksize=required_blocksize;	
		dummy = SG_MALLOC(uint8_t, blocksize+1); /*allow setting of '\0' at EOL*/	
		matrix=SG_MALLOC(SGSparseVector<int64_t>, num_vec);	
		...
		{	
			...
			{	
				...
				{	
					...
					SGSparseVectorEntry<int64_t>* feat=SG_MALLOC(SGSparseVectorEntry<int64_t>, dims);	
					...
					{	
						...
						{	
							...
							{	
								...
								{	
									data[j]='\0';	
									feat[d].entry=(int64_t) m_string_hash((const char*) start);	
									d++;	
									break;	
								}	
							}	
							...
							{	
								data[j]='\0';	
								feat[dims-1].entry=(int64_t) m_string_hash((const char*) start);	
							}	
