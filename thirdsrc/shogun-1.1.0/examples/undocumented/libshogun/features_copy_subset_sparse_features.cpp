...
int main(int argc, char **argv)
{
	...
	for (index_t i=0; i<num_vectors; ++i)
	{
		SG_SPRINT("sparse vector at %d: [", data.sparse_matrix[i].vec_index);
		for (index_t j=0; j<num_features; ++j)
			SG_SPRINT("%d:%f,", data.sparse_matrix[i].features[j].feat_index,data.sparse_matrix[i].features[j].entry);
